...
for ND=1:length(datFiles)    
     ...
     tables1 = tables{ND}(1:7,:);% Truncating Data pairs
     table = [table;tables1];
end
%% Step1: Divide the Input and Output Spaces into Fuzzy Regions
...
%% Step2: Generating Fuzzy Rules from Given Data Pairs
...
%% Testing for different cases of Inputs
    x_input= [10 3 13]; % Input x
    phi_input= [220 -30 30]; % Input phi
for j=1:3
    ...
    for i = 1:iter % no. of iterations for trajectory tracking.
        % value of x.
        mu_xS2_test = trapmf(x_input(j),[0 0 1.5 7]);
        mu_xS1_test = trimf(x_input(j),[4 7 10]);
        ...
        mu_x_test = [mu_xS2_test mu_xS1_test mu_xCE_test mu_xB1_test ...
            mu_xB2_test];
...
ylabel('y (in meters)');
ylim([0 65]);
title('Truck trajectories for the truncated data pairs');
