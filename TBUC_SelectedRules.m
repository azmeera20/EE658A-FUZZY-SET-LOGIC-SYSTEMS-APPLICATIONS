...
%% Step1: Divide the Input and Output spaces of the given numerical 
...
legend('S3','S2','S1','CE','B1','B2','B3');
ylim([0 1]);
%% Step 2: Generating Fuzzy Rules from Given Data Pairs using 14 tables
...
FINAL_RULES=unique(R_temp,'rows','sorted');   
final_rules= FINAL_RULES(12:18,:); 
%It removes redundent rows and sort the Data
...
for j=1:3
    ...
    for i = 1:iter % no. of iterations for trajectory tracking.
        ...
        % value of x.
        mu_x1S2_test = trapmf(x_input(j),[0 0 1.5 7]);
        mu_x2S1_test = trimf(x_input(j),[4 7 10]);
        mu_x3CE_test = trimf(x_input(j),[9 10 11]);
        mu_x4B1_test = trimf(x_input(j),[10 13 16]);
        mu_x5B2_test = trapmf(x_input(j),[13 18.5 20 20]);
        ...
        mu_x_test = [mu_x1S2_test mu_x2S1_test mu_x3CE_test ...
            mu_x4B1_test mu_x5B2_test];
    ...
    % Plot the trajectory.
    plot(x_final,y_final,'.-','MarkerSize',15.0);
